...
def createVideo():
    img_array = []
    for filename in glob.glob('Output_TestVideo_1/*.jpg'):
        img = cv2.imread(filename)
        height, width, layers = img.shape
        size = (width,height)
        img_array.append(img)

    out = cv2.VideoWriter('project1.mp4',cv2.VideoWriter_fourcc(*'DIVX'), 15, size)
...
for i,image_path in enumerate(paths):
    ...
    # plt.subplot(2,3,6)
    # plt.imshow(result)
    mpimg.imsave('Output_TestVideo_1/'+image_path[12:-4]+'_detected.jpg', result)
